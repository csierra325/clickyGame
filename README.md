...
- [Changing the Page `<Music>`](#changing-the-page-Music)
...
## Changing the Page `<Music>`

You can find the source HTML file in the `public` folder of the generated project. You may edit the `<Music>` tag in it to change the Music from “React App” to anything else.

Note that normally you wouldn’t edit files in the `public` folder very often. For example, [adding a stylesheet](#adding-a-stylesheet) is done without touching the HTML.

If you need to dynamically update the page Music based on the content, you can use the browser [`document.Music`](https://developer.mozilla.org/en-US/docs/Web/API/Document/Music) API. For more complex scenarios when you want to change the Music from React components, you can use [React Helmet](https://github.com/nfl/react-helmet), a third party library.

If you use a custom server for your app in production and want to modify the Music before it gets sent to the browser, you can follow advice in [this section](#generating-dynamic-meta-tags-on-the-server). Alternatively, you can pre-build each page as a static HTML file which then loads the JavaScript bundle, which is covered [here](#pre-rendering-into-static-html-files).
...
The `public` folder contains the HTML file so you can tweak it, for example, to [set the page Music](#changing-the-page-Music).
...
```html
<Music>%REACT_APP_WEBSITE_NAME%</Music>
```
...
<html lang="en">
  <head>
    <meta property="og:Music" content="__OG_Music__">
    <meta property="og:description" content="__OG_DESCRIPTION__">
```

Then, on the server, regardless of the backend you use, you can read `index.html` into memory and replace `__OG_Music__`, `__OG_DESCRIPTION__`, and any other placeholders with values depending on the current URL. Just make sure to sanitize and escape the interpolated values so that they are safe to embed into HTML!
